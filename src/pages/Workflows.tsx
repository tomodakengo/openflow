...
const Workflows: React.FC = () => {
  ...
  const handleStatusChange = (id: string, status: 'active' | 'draft' | 'archived') => {
    ...
    if (updatedWorkflow) {
      addToast(`Workflow ${status === 'active' ? 'activated' : status === 'draft' ? 'paused' : 'archived'} successfully`, 'success');
    } else {
                ...
                <tr key={workflow.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full
                      ${workflow.status === 'active' ? 'bg-success-100 text-success-800' : ''}
                      ${workflow.status === 'draft' ? 'bg-gray-100 text-gray-800' : ''}
                      ${workflow.status === 'archived' ? 'bg-warning-100 text-warning-800' : ''}
                    `}>
