...
const Dashboard: React.FC = () => {
  ...
  const priorityData = [
    { name: 'High', value: tasksByPriority.find(t => t.priority === 'high')?.count || 0 },
    { name: 'Medium', value: tasksByPriority.find(t => t.priority === 'medium')?.count || 0 },
    { name: 'Low', value: tasksByPriority.find(t => t.priority === 'low')?.count || 0 },
  ];
  
  const statusData = [
    { name: 'To Do', value: tasksByStatus.find(t => t.status === 'todo')?.count || 0 },
    { name: 'In Progress', value: tasksByStatus.find(t => t.status === 'in_progress')?.count || 0 },
    { name: 'Completed', value: tasksByStatus.find(t => t.status === 'completed')?.count || 0 },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex justify-between items-center mb-4">
            ...
            <a href="/workflows" className="text-primary-600 hover:text-primary-800 text-sm flex items-center">
              View all <ArrowRight className="h-4 w-4 ml-1" />
            </a>
          </div>
          <div className="space-y-4">
            {recentWorkflows.map(workflow => (
              <div key={workflow.id} className="flex items-center p-3 border rounded-md hover:bg-gray-50 transition-colors">
                ...
                <a href={`/workflows/${workflow.id}`} className="text-primary-600 hover:text-primary-800">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex justify-between items-center mb-4">
            ...
            <a href="/tasks" className="text-primary-600 hover:text-primary-800 text-sm flex items-center">
              View all <ArrowRight className="h-4 w-4 ml-1" />
            </a>
          </div>
          <div className="space-y-4">
            {myTasks.length > 0 ? (
              myTasks.slice(0, 3).map(task => (
                <div key={task.id} className="flex items-center p-3 border rounded-md hover:bg-gray-50 transition-colors">
                  <div className="mr-3">
